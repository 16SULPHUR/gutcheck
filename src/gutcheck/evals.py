...
def render_markdown(report: dict[str, Any], pack: Pack) -> str:
    lines = [
        ...
        *(
            [
                f"Answers come from [{pack.model.repo}](https://huggingface.co/{pack.model.repo})"
                f" @ `{pack.model.revision[:10]}`, fine-tuned for this pack. Calibrated rows use a",
                "temperature fitted on rows held out from its training; the test rows below were",
                "never seen in training or fitting.",
            ]
            if pack.model
            else [
                "Calibrated rows use a temperature fitted on each dataset's separate training",
                "split, so the test rows below were never seen during fitting. Laya's base",
                "checkpoints are not trained on these tasks; the numbers show what you get out",
                "of the box.",
            ]
        ),
        "",
    ...
    for qid, q in report["questions"].items():
        ds = q["dataset"]
        source = (
            f"[{ds['repo']}](https://huggingface.co/"
            f"{'datasets/' if ds.get('repo_type', 'dataset') == 'dataset' else ''}{ds['repo']})"
            if ds["repo"]
            else ds["path"]
        )
